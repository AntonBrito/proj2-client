...
module.exports = {
  host: 'https://pure-beach-11955.herokuapp.com/'
};
...
var memberLogin = function memberLogin() {
...
var setAPIOrigin = function setAPIOrigin(location, config) {
  ...
  if (search.environment === 'development' || location.hostname === 'localhost' && search.environment !== 'production') {
    if (!(config.apiOrigin = config.apiOrigins.development)) {
      ...
      config.apiOrigin = 'https://pure-beach-11955.herokuapp.com/';
    }
  } else {
...
var config = {
  apiOrigins: {
    production: 'https://pure-beach-11955.herokuapp.com/'
  }
};
...
var onSignUp = function onSignUp(event) {
  ...
  api.signUp(data).done(ui.onSignUpSuccess).fail(ui.onSignUpFailure);
};

var onChangePassword = function onChangePassword(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.changePassword(data).done(ui.changePasswordSuccess).fail(ui.onChangePasswordFail);
};
...
var onGetGames = function onGetGames(event) {
  event.preventDefault();
  var data = app.user.game;
  console.log(data);
  api.index(data).then(ui.getSuccess).catch(ui.fail);
...
var onSignIn = function onSignIn(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.signIn(data).then(ui.onSignInSuccess).then(function () {
...
var onCreateGame = function onCreateGame(event) {
  event.preventDefault();
  var nameText = $('#gName').val();
  var platformText = $('#gPlatform').val();
  var genreText = $('#gGenre').val();
  var yearText = $('#gYear').val();
  api.createGame(nameText, platformText, genreText, yearText).then(ui.onCreateGameSuccess).then(function () {
    return onGetGames(event);
  }).catch(ui.fail);
};

var onEditGame = function onEditGame(event) {
  event.preventDefault();
  var idText = $('#egId').val();
  var nameText = $('#egName').val();
  var platformText = $('#egPlatform').val();
  var genreText = $('#egGenre').val();
  var yearText = $('#egYear').val();
  api.editGame(idText, nameText, platformText, genreText, yearText).then(ui.onEditGameSuccess).then(function () {
    return onGetGames(event);
  }).catch(ui.fail);
};
...
var onDeleteGame = function onDeleteGame(event) {
  event.preventDefault();
  var idText = $('#dgId').val();
  api.deleteGame(idText).then(ui.onDeleteGameSuccess).then(function () {
    return onGetGames(event);
  }).catch(ui.fail);
};
...
var createGame = function createGame(nameText, platformText, genreText, yearText) {
  return $.ajax({
    url: app.host + '/games',
    method: 'POST',
    headers: {
...
var editGame = function editGame(idText, nameText, platformText, genreText, yearText) {
  return $.ajax({
    url: app.host + '/games/' + app.user.id,
    method: 'PATCH',
    headers: {
...
var deleteGame = function deleteGame(idText) {
  return $.ajax({
    url: app.host + '/games/' + app.user.id,
    method: 'DELETE',
    headers: {
...
var onSignInFail = function onSignInFail() {
...
var onChangePasswordFail = function onChangePasswordFail() {
...
var loopGames = function loopGames(data) {
  for (var i = 0; i < data.games.length; i++) {
    $('#gameStats').append('<tr class="gameRow">' + '<td>' + data.games[i].id + '</td>' + '<td>' + data.games[i].game_name + '</td>' + '<td>' + data.games[i].game_platform + '</td>' + '<td>' + data.games[i].game_genre + '</td>' + '<td>' + data.games[i].game_year + '</td>' + '</tr>');
  }
};
...
var onEditGameSuccess = function onEditGameSuccess(data) {
  // app.game = data.game
  // app.game.id = data.game.id
};

var onDeleteGameSuccess = function onDeleteGameSuccess(data) {
  // app.game = data.game
  // app.game.id = data.game.id
};
...
module.exports = {
  ...
  onEditGameSuccess: onEditGameSuccess,
  onDeleteGameSuccess: onDeleteGameSuccess,
